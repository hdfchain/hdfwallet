...
import (
	...
	gcs2 "github.com/hdfchain/hdfd/gcs/v2"
...
// message or hdfd JSON-RPC request, for the blocks in sidechain and saved in
...
// spending to it in the blockchain or hdfd mempool), the next chained address
// is returned.
...
func makeTicketSummary(ctx context.Context, rpc *hdfd.RPC, dbtx walletdb.ReadTx,
...
func (w *Wallet) LockedOutpoints(ctx context.Context, accountName string) ([]hdfdtypes.TransactionInput, error) {
	w.lockedOutpointMu.Lock()
	...
	acctLocked := make([]hdfdtypes.TransactionInput, 0, len(allLocked))
	err := walletdb.View(ctx, w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		for i := range allLocked {
			...
			acctLocked = append(acctLocked, hdfdtypes.TransactionInput{
...
func ticketMatured(params *chaincfg.Params, txHeight, curHeight int32) bool {
	// hdfd has an off-by-one in the calculation of the ticket
...
// returning the total amount of hdfchain received for all accounts.
...
// returning the total amount of hdfchain received for a single wallet
// address.
