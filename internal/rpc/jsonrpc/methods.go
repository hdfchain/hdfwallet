...
func (s *Server) getBestBlock(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	result := &hdfdtypes.GetBestBlockResult{
		Hash:   hash.String(),
		Height: int64(height),
	}
	return result, nil
...
func (s *Server) getBlockHash(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hdfdtypes.GetBlockHashCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func (s *Server) getInfo(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if rpc, ok := n.(*hdfd.RPC); ok {
		var consensusInfo hdfdtypes.InfoChainResult
...
// appear in the blockchain, or any tx that has arrived in the hdfd mempool).
...
func (s *Server) getPeerInfo(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	if !ok {
		var resp []*hdfdtypes.GetPeerInfoResult
		if rpc, ok := n.(*hdfd.RPC); ok {
...
func (s *Server) help(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hdfdtypes.HelpCmd)
	// TODO: The "help" RPC should use a HTTP POST client when calling down to
	// hdfd for additional help methods.  This avoids including websocket-only
...
func (s *Server) ticketsForAddress(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hdfdtypes.TicketsForAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	return hdfdtypes.TicketsForAddressResult{Tickets: ticketHashStrs}, nil
}
...
func (s *Server) signRawTransaction(ctx context.Context, icmd interface{}) (interface{}, error) {
	...
	// TODO: really we probably should look these up with hdfd anyway to
	...
	// querying hdfd with getrawtransaction. We queue up a bunch of async
	// requests and will wait for replies after we have checked the rest of
	// the arguments.
	requested := make(map[wire.OutPoint]*hdfdtypes.GetTxOutResult)
	...
	if rpc, ok := n.(*hdfd.RPC); ok {
		for i, txIn := range tx.TxIn {
			...
			requestedGroup.Go(func() error {
				...
				var res *hdfdtypes.GetTxOutResult
				err := rpc.Call(gctx, "gettxout", &res, hash, index, true)
				if err != nil {
					return errors.E(errors.Op("hdfd.jsonrpc.gettxout"), err)
				}
				requestedMu.Lock()
...
func (s *Server) validateAddress(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hdfdtypes.ValidateAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func (s *Server) verifyMessage(ctx context.Context, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hdfdtypes.VerifyMessageCmd)
...
func (s *Server) version(ctx context.Context, icmd interface{}) (interface{}, error) {
	resp := make(map[string]hdfdtypes.VersionResult)
	n, _ := s.walletLoader.NetworkBackend()
	if rpc, ok := n.(*hdfd.RPC); ok {
	...
	resp["hdfwalletjsonrpcapi"] = hdfdtypes.VersionResult{
