...
// notifications from a hdfd JSON-RPC server.
type Syncer struct {
...
// websocket connection to a hdfd JSON-RPC server.
...
// NewSyncer creates a Syncer that will sync the wallet using hdfd JSON-RPC.
...
// the hdfd server are performed.  Otherwise, it will listen for notifications
...
func (s *Syncer) winningTickets(ctx context.Context, params json.RawMessage) error {
	block, height, winners, err := hdfd.WinningTickets(params)
	if err != nil {
		return err
	}
...
func (s *Syncer) blockConnected(ctx context.Context, params json.RawMessage) error {
	header, relevant, err := hdfd.BlockConnected(params)
	if err != nil {
		return err
	}
...
func (s *Syncer) relevantTxAccepted(ctx context.Context, params json.RawMessage) error {
	tx, err := hdfd.RelevantTxAccepted(params)
	if err != nil {
		return err
	}
...
func (s *Syncer) spentAndMissedTickets(ctx context.Context, params json.RawMessage) error {
	missed, err := hdfd.MissedTickets(params)
	if err != nil {
		return err
	}
