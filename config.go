...
import (
	...
	"github.com/hdfchain/hdfwallet/internal/cfgutil"
	"github.com/hdfchain/hdfwallet/internal/netparams"
	"github.com/hdfchain/hdfd/connmgr/v2"
	"github.com/hdfchain/hdfd/dcrutil/v2"
	"github.com/hdfchain/hdfwallet/errors/v2"
	"github.com/hdfchain/hdfwallet/version"
	"github.com/hdfchain/hdfwallet/wallet/v3"
	"github.com/hdfchain/hdfwallet/wallet/v3/txrules"
...
var (
	dcrdDefaultCAFile  = filepath.Join(dcrutil.AppDataDir("hdfd", false), "rpc.cert")
	defaultAppDataDir  = dcrutil.AppDataDir("hdfwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Network address of hdfd RPC server"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"hdfd RPC Certificate Authority"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for hdfd RPC; only allowed when connecting to localhost"`
	DcrdUsername     string                  `long:"dcrdusername" description:"hdfd RPC username; overrides --username"`
	DcrdPassword     string                  `long:"dcrdpassword" default-mask:"-" description:"hdfd RPC password; overrides --password"`
	...
	NoDcrdProxy  bool   `long:"nodcrdproxy" description:"Never use configured proxy to dial hdfd websocket connectons"`
	...
	Username               string                  `short:"u" long:"username" description:"JSON-RPC username and default hdfd RPC username"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"JSON-RPC password and default hdfd RPC password"`
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local hdfd cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local hdfd and switch to its RPC cert if it exists.
	...
	// If the hdfd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for hdfd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for hdfd and wallet.
	if cfg.DcrdUsername == "" {
