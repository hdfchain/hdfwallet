...
var (
	hdfdDefaultCAFile  = filepath.Join(dcrutil.AppDataDir("hdfd", false), "rpc.cert")
...
type config struct {
	...
	DcrdUsername     string                  `long:"hdfdusername" description:"hdfd RPC username; overrides --username"`
	DcrdPassword     string                  `long:"hdfdpassword" default-mask:"-" description:"hdfd RPC password; overrides --password"`
	...
	NoDcrdProxy  bool   `long:"nohdfdproxy" description:"Never use configured proxy to dial hdfd websocket connectons"`
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		if !cfg.CAFile.ExplicitlySet() {
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					hdfdCertExists, err := cfgutil.FileExists(
						hdfdDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if hdfdCertExists {
						cfg.CAFile.Value = hdfdDefaultCAFile
					}
				}
			}
		}
